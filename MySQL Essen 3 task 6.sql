...
VALUES
 ...
 SELECT 		 
         OrderID as №_замовлення, 
         CONCAT(lastNameC, ' ', firstNameC) as  Покупець,         
         sum(OrderDetails.totalQty * store.price) AS TotalPrice,
         COUNT(OrderID) as Кількість_позицій,    
         position as Посада_працівника,
         CONCAT(firstName, ' ', lastName) as Працівник         
            
 FROM ShopDB.Orders2
 
 JOIN ShopDB.employee
 ON employee.Employeeid = orders2.IDEmployee
 
 JOIN ShopDB.customer
 ON customer.Customerid = Orders2.IDcustomer
 
 JOIN ShopDB.store
 JOIN ShopDB.OrderDetails 
 ON OrderDetails.IDstore = store.storeid
 ON OrderDetails.DetailsOrderID = Orders2.OrderID 
